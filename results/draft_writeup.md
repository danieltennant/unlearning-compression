...
**TL;DR:** This was my first AI safety research project, produced as part of the [BlueDot Impact Technical AI Safety](https://bluedot.org/courses/technical-ai-safety) program sprint, so I'd love to hear feedback.  In this project I set out to first replicate, and then extend a previous paper that showed Machine Unlearning could be reversed by standard model compression techniques. I show that 4-bit quantization and magnitude pruning both reverse unlearning across three different methods (GradDiff, SimNPO, RMU) on the TOFU benchmark — recovering suppressed knowledge while leaving model utility intact. Weight-level analysis suggests the reason: unlearning perturbations are small and concentrated in low-magnitude weights, exactly the weights that compression removes first.
